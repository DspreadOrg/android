...
public class PrintTicketActivity extends PrinterBaseActivity<ActivityPrinterBaseBinding, PrintTicketViewModel> {
    ...
    protected void onReturnPrintResult(boolean isSuccess, String status, PrinterDevice.ResultType resultType) {
        ...
        if (isSuccess) {
            dialog(PrintTicketActivity.this, "Print Successful", 5000L, true, false);
        } else {
            dialog(PrintTicketActivity.this, "Print Fail", 5000L, false, true);

        }
    }

    private static void dialog(Context mContext, String message, Long duration, boolean isShowCountdown, boolean isShowCloseButton) {
